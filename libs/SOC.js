...
require('../app/models/Chats.js');
require('../app/models/Rooms.js');
//using models.
...
module.exports.sockets = function(http){
  ...
  ioChat.on('connection', function(socket){
    ...
    oldChats = function(result,username,room){
      ioChat.to(userSocket[username]).emit('old-chats',{result:result,room:room});
    }
    ...
    socket.on('chat-msg', function(data){
      ...
      ioChat.to(socket.room).emit('chat-msg',{msgFrom:socket.username,msg:data.msg,date:data.date});
    });
